...
public class UnitOfWorkTests(ITestOutputHelper output) : NHibernateTestsBase(output)
{
    ...
    public async Task EnsureQueriedEntitiesAreTracked()
    {
        ...
        var queriedUser = await _npgsqlClient.QuerySingleOrDefaultAsync<(string, string, string, bool)>("SELECT * FROM Users;");
    ...
    public async Task Add()
    {
        ...
        var shouldNull = await _npgsqlClient.QuerySingleOrDefaultAsync<(string, string, string, bool)?>("SELECT * FROM Users;");
        Assert.Null(shouldNull);

        await unitOfWork.SaveChangesAsync(default);
        var shouldSaved = await _npgsqlClient.QuerySingleOrDefaultAsync<(string, string, string, bool)>("SELECT * FROM Users;");
    ...
    public async Task Transaction()
    {
        ...
        await unitOfWork.SaveChangesAsync(default);
        Assert.False(await unitOfWork.HasChangesAsync(default));
        var shouldNullBecauseUncommitTsx = await _npgsqlClient.QuerySingleOrDefaultAsync<(string, string, string, bool)?>("SELECT * FROM Users;");
        ...
        var shouldChanged = await _npgsqlClient.QuerySingleOrDefaultAsync<(string, string, string, bool)?>("SELECT * FROM Users;");
    ...
    public async Task Rollback()
    {
        ...
        var userSaved = await _npgsqlClient.QuerySingleOrDefaultAsync<(string, string, string, bool)>("SELECT * FROM Users;");
