...
public partial class CommandExecutorTests
{
    ...
    public async Task MiddlewareIsInvoked_Success()
    {
        ...
        Container.Clear();
        Container.Services.AddTransient<ICommandExecutor, CommandExecutor>(_ => new CommandExecutor(new ICommandMiddleware[] { new TestMiddleware() }));
    ...
    public async Task MiddlewarePipelineExecutesInOrder_Success()
    {
        ...
        Container.Clear();
        Container.Services.AddTransient<ICommandExecutor, CommandExecutor>(_ =>
            new CommandExecutor(new ICommandMiddleware[] { new TestMiddleware(), new TestMiddleware2() }));
        Container.Initialize();
    ...
    public async Task CommandContextIsOk_Success()
    {
        ...
        Container.Clear();
        Container.Services.AddTransient<ICommandExecutor, CommandExecutor>(_ =>
            new CommandExecutor(new ICommandMiddleware[] { new AssertContextMiddlewareBeforeCommand(), new AssertContextMiddlewareAfterCommand() }));
        Container.Initialize();
